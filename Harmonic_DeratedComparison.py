...
thd=pd.DataFrame(index=comps,columns=cases,dtype=(float))
passes={}
for i in cases:  
    passes[i]=pd.DataFrame(columns=comps)
    ...
    for c in comps:
        ...
        if c=='CC':
            dssText.Command = "New Load.LOAD1 Phases=1 Model=5 Status=1 Bus1=3.1 kV=0.230 kW="+str(EV_power_cc[i+'_CC'])+" PF=1 spectrum="+str(i)+'_CC'
            dssText.Command = "New Load.LOAD2 Phases=1 Model=5 Status=1 Bus1=3.1 kV=0.230 kW="+str(EV_power_cc[i+'_CC'])+" PF=1 spectrum="+str(i)+'_CC'
        ...
        if c.split('_')[0]=="De":
            ...
            for q in range(1,n_de+1):
                dssText.Command = "New Load.LOAD"+str(q)+" Model=5 Phases=1 Status=1 Bus1=3.1 kV=0.230 kW="+str(EV_power_de[i.lower()+'_'+str(amps)]/cf)+" PF=1 spectrum="+str(i.lower()+'_'+str(amps))
        ...
        thd[i.split('_')[0]][c]=sum(Vh_ratios['V_ratio'][1:]**2)**0.5
        passes[i.split('_')[0]][c] = Vh_ratios['V_ratio'][1:]<Vh_ratios['Lims'][1:]
        passes[i.split('_')[0]]['h']=res_Reactor[' Harmonic']
        x=Vh_ratios['h'][1:]
    ...
    plt.tight_layout()
    plt.savefig('figs/'+i+'_Derated_MultipleCars.png')
